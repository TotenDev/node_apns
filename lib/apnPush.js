...
function Push(tls_opts) {
  ...
  this.nextUID = function () { return this.uid++; }  
  return this;
}
...
Push.prototype.connect = function connect(callback) {
  ...
  try {
    this.cleartextStream = tls.connect(instance.options.port, instance.options.host, instance.options.tls, function () {
      ...
      if (!instance.cleartextStream.authorized) { 
    ...
    //data
    this.cleartextStream.on('data', function (data) {
        // Bytes  --  |  1  |    1    |        4        |
    ...
    //error close
    this.cleartextStream.on('timeout', function () { instance.respondError("connection timed out",instance); });
    this.cleartextStream.on('clientError', function(exception) { instance.respondError("Error in establishing connection with apple push server (" + err + ") --  host: " + instance.options.host + ":" + instance.options.port,instance); });
    this.cleartextStream.on('error', function (exception) { instance.respondError("Error in connecting with apple push server (" + err + ") --  host: " + instance.options.host + ":" + instance.options.port,instance); });
    //normal close
    this.cleartextStream.on('close', function () { instance.emit('end'); instance.disconnect(); });
    this.cleartextStream.on('end', function () { instance.emit('end'); instance.disconnect(); });
