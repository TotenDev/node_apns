...
Feedback.prototype.connect = function connect(tls_opts) {
  ...
  try {
    this.cleartextStream = tls.connect(this.options.port, this.options.host, tls_opts, function () {
      definitions.log('Feedback from ' + instance.options.host + ':' + instance.options.port);
      ...
      if (!instance.cleartextStream.authorized) { 
        ...
        instance.cleartextStream.on('data', function (data) { instance.receiveData(data); });
      }
    });
    ...
    this.cleartextStream.on('timeout', function () { instance.respondError("Error in establishing connection with apple feedback server (timedout) --  host: " + instance.options.host + ":" + instance.options.port,instance); });
