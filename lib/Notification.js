...
Notification.prototype.normalizedPayload = function () {
    ...
    if (typeof(this.badge) === 'number' || typeof(this.badge) === 'string') normalized.aps.badge = this.badge;
    return normalized;
}
...
/* Custom Getters*/
Notification.__defineGetter__("alert", function () {
  if (this.payload && this.payload["apn"] && this.payload.apn["alert"] && this.payload.apn.alert.length > 0) { return this.payload.apn["alert"]; }
  else if (this.alert && this.alert.length > 0) { return this.alert; }
  return null;
})
Notification.__defineGetter__("sound", function () {
  if (this.payload && this.payload["apn"] && this.payload.apn["sound"] && this.payload.apn.sound.length > 0) { return this.payload.apn["sound"]; }
  else if (this.sound && this.sound.length > 0) { return this.sound; }
  return null;
})
Notification.__defineGetter__("badge", function () {
  if (this.payload && this.payload["apn"] && this.payload.apn["badge"] && this.payload.apn.badge.length > 0) { return this.payload.apn["badge"]; }
  else if (this.badge && this.badge.length > 0) { return this.badge; }
  return null;
})